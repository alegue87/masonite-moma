...
class Kernel:
    ...
    def register(self):
        ...
        self.register_scheduler_hr()

    def register_scheduler_hr(self):
        '''
            Is registered only when used with gunicorn web server or craft and
            accept only one worker for instance, like:

            gunicorn -w 1 wsgi:application --bind localhost:8080
        '''

        if 'gunicorn' in sys.argv[0]:
            SchedulerHighRate(self.application)
            return
        
        if len(sys.argv) > 1 and 'serve' in sys.argv[1]:
            SchedulerHighRate(self.application)
            return 
